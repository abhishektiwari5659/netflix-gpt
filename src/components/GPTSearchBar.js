...
const GPTSearchBar = () => {
  ...
  try {
    ...
    // Return exact match or fallback to first result
  ...
  // Remove null results if TMDB returned nothing
  const finalMovies = tmdbResult.filter(Boolean);
  console.log("Final TMDB results (exact match or fallback):", finalMovies);
};
